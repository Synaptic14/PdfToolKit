...
final class ImportedCatalogMetadataPreservationTest extends TestCase
{
    public function testImportedCatalogMetadataStreamIsPreservedOnSave(): void
    {
        $xmp = '<?xpacket begin=""?><x:xmpmeta><pdf:Producer>Fixture</pdf:Producer></x:xmpmeta>';
        $pdf = $this->buildPdf([
        ...
        $this->assertStringContainsString('<pdf:Producer>Fixture</pdf:Producer>', $bytes);
    }

    public function testInlineCatalogMetadataIsPreservedOnSave(): void
    {
        $pdf = $this->buildPdf([
            1 => '<< /Type /Catalog /Pages 2 0 R /Metadata << /Type /Metadata /Subtype /XML >> >>',
            2 => '<< /Type /Pages /Kids [3 0 R] /Count 1 /MediaBox [0 0 200 400] >>',
            3 => '<< /Type /Page /Parent 2 0 R /Contents 4 0 R >>',
            4 => "<< /Length 0 >>\nstream\n\nendstream",
        ]);
        ...
        $this->assertStringContainsString('/Metadata', $bytes);
        $this->assertStringContainsString('/Subtype /XML', $bytes);
        $this->assertStringContainsString('/Type /Metadata', $bytes);
    }
