...
final class ImportedDocumentLanguagePreservationTest extends TestCase
{
    public function testImportedDocumentLanguageIsPreservedOnSave(): void
    {
        $pdf = $this->buildPdf([
            1 => '<< /Type /Catalog /Pages 2 0 R /Lang (de-DE) >>',
        ...
        $this->assertSame('de-DE', $imported->document()->language());

        $bytes = $imported->save();

        $this->assertStringContainsString('/Lang (de-DE)', $bytes);
    }
