...
final class ImportedUriBasePreservationTest extends TestCase
{
    public function testImportedUriBaseIsPreservedOnSave(): void
    {
        $pdf = $this->buildPdf([
            1 => '<< /Type /Catalog /Pages 2 0 R /URI << /Base (https://docs.example/) >> >>',
        ...
        $this->assertSame('https://docs.example/', $imported->document()->uriBase());

        $bytes = $imported->save();

        $this->assertStringContainsString('/URI << /Base (https://docs.example/) >>', $bytes);
    }
