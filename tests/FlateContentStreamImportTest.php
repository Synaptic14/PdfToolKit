...
final class FlateContentStreamImportTest extends TestCase
{
    public function testFlateDecodedPageContentCanBeParsedAndRewritten(): void
    {
        if (!function_exists('zlib_encode')) {
            $this->markTestSkipped('zlib extension is required for FlateDecode fixtures.');
        }
        ...
        $pdf = $this->buildPdf([
            ...
            4 => "<< /Length " . strlen($encoded) . " /Filter /FlateDecode >>\nstream\n" . $encoded . "\nendstream",
        ]);
        ...
        $this->assertStringContainsString('(New text) Tj', $bytes);
        $this->assertStringNotContainsString('/Filter /FlateDecode', $bytes);
    }

    public function testFlateDecodeAbbreviationCanBeParsed(): void
    {
        if (!function_exists('zlib_encode')) {
            $this->markTestSkipped('zlib extension is required for FlateDecode fixtures.');
        }
        ...
        $pdf = $this->buildPdf([
            ...
            2 => '<< /Type /Pages /Kids [3 0 R] /Count 1 /MediaBox [0 0 200 400] >>',
            3 => '<< /Type /Page /Parent 2 0 R /Contents 4 0 R >>',
            4 => "<< /Length " . strlen($encoded) . " /Filter /Fl >>\nstream\n" . $encoded . "\nendstream",
        ]);
