...
final class IndirectStreamLengthTest extends TestCase
{
    public function testParserSupportsIndirectStreamLengthObjects(): void
    {
        $content = "BT\nET";
        $pdf = $this->buildPdf([
            ...
            4 => "<< /Length 5 0 R >>\nstream\n" . $content . "\nendstream",
            5 => (string) strlen($content),
        ]);

        $parsed = (new PdfParser())->parseString($pdf);

        $this->assertSame($content, $parsed->pages()[0]->contentStreams[0]->contents);
    }
